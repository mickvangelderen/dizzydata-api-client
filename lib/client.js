...
var DizzydataApiClient = module.exports = function(options) {
	...
	// attach certificates for secure api endpoints
	if (url.parse(this.baseUrl).protocol === 'https:') {
...
DizzydataApiClient.prototype._authorizedRequest = function(options) {
	...
	return self.token.then(req).catch(function(error) {
		...
		if (error.statusCode && error.statusCode === 401 || error.body.StatusCode === 401) {
			// remove the token
			self.token = null;
			// try again
